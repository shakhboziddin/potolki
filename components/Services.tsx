      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {t.services.items.map((item, index) => (
            <div 
              key={index} 
              className={`group bg-white rounded-t-2xl rounded-b-2xl shadow-lg hover:shadow-2xl transition-all duration-300 flex flex-col h-full border border-slate-100 reveal reveal-delay-${(index % 3) + 1}`}
            >
              {/* Image Area */}
